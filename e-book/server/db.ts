export const BOOKS: any = {

  17: {
    id: 17,
    description: "Reactive SPA",
    longDescription: "How to build SPA applications in Reactive style using plain RxJs - Patterns and Anti-Patterns",
    iconUrl: "https://via.placeholder.com/150",
    bookIconUrl: "https://via.placeholder.com/32",
    category: "BEGINNER",
    articlesCount: 10,
    seqNo: 0,
    url: "reactive-angular",
    price: 50
  },

  3: {
    id: 3,
    description: "RxJs In Practice",
    longDescription: "Understand the RxJs Observable pattern, learn the RxJs Operators via practical examples",
    iconUrl: "https://via.placeholder.com/150",
    bookIconUrl: "https://via.placeholder.com/32",
    category: "BEGINNER",
    articlesCount: 10,
    seqNo: 2,
    url: "rxjs",
    price: 50
  },

  4: {
    id: 4,
    description: "NgRx (with NgRx Data) - The Complete Guide",
    longDescription: "Learn the modern Ngrx Ecosystem, including NgRx Data, Store, Effects, Router Store, Ngrx Entity, and Dev Tools.",
    iconUrl: "https://via.placeholder.com/150",
    category: "BEGINNER",
    articlesCount: 10,
    seqNo: 1,
    url: "ngrx",
    promo: false,
    price: 50
  },

  2: {
    id: 2,
    description: "SPA Core Deep Dive",
    longDescription: "A detailed walk-through of the most important part of SPA - the Core and Common modules",
    iconUrl: "https://via.placeholder.com/150",
    articlesCount: 10,
    category: "BEGINNER",
    seqNo: 3,
    url: "angular-core",
    price: 50
  },

  5: {
    id: 5,
    description: "SPA for Beginners",
    longDescription: "Establish a solid layer of fundamentals, learn what\'s under the hood of SPA",
    iconUrl: "https://via.placeholder.com/150",
    bookIconUrl: "https://via.placeholder.com/32",
    category: "BEGINNER",
    articlesCount: 10,
    seqNo: 4,
    url: "angular-for-beginners",
    price: 50
  },

  12: {
    id: 12,
    description: "SPA Testing",
    longDescription: "In-depth guide to Unit Testing and E2E Testing of SPA Applications",
    iconUrl: "https://via.placeholder.com/150",
    category: "BEGINNER",
    seqNo: 5,
    url: "angular-testing",
    articlesCount: 10,
    promo: false,
    price: 50
  },

  1: {
    id: 1,
    description: "Serverless SPA with Firebase",
    longDescription: "Serveless SPA with Firestore, Firebase Storage & Hosting, Firebase Cloud Functions & SPAFire",
    iconUrl: "https://via.placeholder.com/150",
    articlesCount: 10,
    category: "BEGINNER",
    seqNo: 6,
    url: "serverless-angular",
    price: 50
  },

  16: {
    id: 16,
    description: "Stripe Payments In Practice",
    longDescription: "Build your own ecommerce store & membership website with Firebase, Stripe and Express",
    iconUrl: "https://via.placeholder.com/150",
    articlesCount: 10,
    category: "BEGINNER",
    seqNo: 7,
    url: "stripe",
    price: 50
  },

  14: {
    id: 14,
    description: "NestJs In Practice (with MongoDB)",
    longDescription: "Build a modern REST backend using Typescript, MongoDB and the familiar SPA API.",
    iconUrl: "https://via.placeholder.com/150",
    category: "BEGINNER",
    articlesCount: 10,
    seqNo: 8,
    url: "nestjs",
    promo: false,
    price: 50
  },

  6: {
    id: 6,
    description: "SPA Security - Web Security Fundamentals",
    longDescription: "Learn Web Security Fundamentals and apply them to defend an SPA / Node Application from multiple types of attacks.",
    iconUrl: "https://via.placeholder.com/150",
    bookIconUrl: "https://via.placeholder.com/32",
    category: "ADVANCED",
    articlesCount: 11,
    seqNo: 9,
    url: "angular-security",
    price: 50
  },

  7: {
    id: 7,
    description: "SPA PWA - Progressive Web Apps",
    longDescription: "Learn SPA Progressive Web Applications, build the future of the Web Today.",
    iconUrl: "https://via.placeholder.com/150",
    bookIconUrl: "https://via.placeholder.com/32",
    category: "ADVANCED",
    articlesCount: 8,
    seqNo: 10,
    url: "angular-pwa",
    price: 50
  },

  8: {
    id: 8,
    description: "SPA Advanced Library Laboratory: Build Your Own Library",
    longDescription: "Learn Advanced SPA functionality typically used in Library Development. Advanced Components, Directives, Testing, Npm",
    iconUrl: "https://via.placeholder.com/150",
    bookIconUrl: "https://via.placeholder.com/32",
    category: "ADVANCED",
    seqNo: 11,
    url: "angular-advanced",
    price: 50
  },

  9: {
    id: 9,
    description: "The Complete Typescript",
    longDescription: "Complete Guide to Typescript From Scratch: Learn the language in-depth and use it to build a Node REST API.",
    iconUrl: "https://via.placeholder.com/150",
    bookIconUrl: "https://via.placeholder.com/32",
    category: "BEGINNER",
    seqNo: 12,
    url: "typescript",
    price: 50
  },

  11: {
    id: 11,
    description: "SPA Material",
    longDescription: "Build Applications with the official SPA Widget Library",
    iconUrl: "https://via.placeholder.com/150",
    category: "BEGINNER",
    seqNo: 14,
    url: "angular-material",
    price: 50
  }

};

export const ARTICLES = {

  1: {
    id: 1,
    description: "SPA Tutorial For Beginners - Build Your First App - Hello World Step By Step",
    duration: "4:17",
    seqNo: 1,
    bookId: 5
  },

  2: {
    id: 2,
    description: "Building Your First  Component - Component Composition",
    duration: "2:07",
    seqNo: 2,
    bookId: 5
  },

  3: {
    id: 3,
    description: "Component @Input - How To Pass Input Data To an  Component",
    duration: "2:33",
    seqNo: 3,
    bookId: 5
  },

  4: {
    id: 4,
    description: " Component Events - Using @Output to create custom events",
    duration: "4:44",
    seqNo: 4,
    bookId: 5
  },

  5: {
    id: 5,
    description: " Component Templates - Inline Vs External",
    duration: "2:55",
    seqNo: 5,
    bookId: 5
  },

  6: {
    id: 6,
    description: "Styling  Components - Learn About Component Style Isolation",
    duration: "3:27",
    seqNo: 6,
    bookId: 5
  },

  7: {
    id: 7,
    description: " Component Interaction - Extended Components Example",
    duration: "9:22",
    seqNo: 7,
    bookId: 5
  },

  8: {
    id: 8,
    description: " Components Tutorial For Beginners - Components Exercise !",
    duration: "1:26",
    seqNo: 8,
    bookId: 5
  },

  9: {
    id: 9,
    description: " Components Tutorial For Beginners - Components Exercise Solution Inside",
    duration: "2:08",
    seqNo: 9,
    bookId: 5
  },

  10: {
    id: 10,
    description: " Directives - Inputs, Output Event Emitters and How To Export Template References",
    duration: "4:01",
    seqNo: 10,
    bookId: 5
  },

  // Security
  11: {
    id: 11,
    description: "Course Helicopter View",
    duration: "08:19",
    seqNo: 1,
    bookId: 6
  },

  12: {
    id: 12,
    description: "Installing Git, Node, NPM and Choosing an IDE",
    duration: "04:17",
    seqNo: 2,
    bookId: 6
  },

  13: {
    id: 13,
    description: "Installing The Articles Code - Learn Why Its Essential To Use NPM 5",
    duration: "06:05",
    seqNo: 3,
    bookId: 6
  },

  14: {
    id: 14,
    description: "How To Run Node In TypeScript With Hot Reloading",
    duration: "03:57",
    seqNo: 4,
    bookId: 6
  },

  15: {
    id: 15,
    description: "Guided Tour Of The Sample Application",
    duration: "06:00",
    seqNo: 5,
    bookId: 6
  },

  16: {
    id: 16,
    description: "Client Side Authentication Service - API Design",
    duration: "04:53",
    seqNo: 6,
    bookId: 6
  },

  17: {
    id: 17,
    description: "Client Authentication Service - Design and Implementation",
    duration: "09:14",
    seqNo: 7,
    bookId: 6
  },

  18: {
    id: 18,
    description: "The New SPA HTTP Client - Doing a POST Call To The Server",
    duration: "06:08",
    seqNo: 8,
    bookId: 6
  },

  19: {
    id: 19,
    description: "User Sign Up Server-Side Implementation in Express",
    duration: "08:50",
    seqNo: 9,
    bookId: 6
  },

  20: {
    id: 20,
    description: "Introduction To Cryptographic Hashes - A Running Demo",
    duration: "05:46",
    seqNo: 10,
    bookId: 6
  },

  21: {
    id: 21,
    description: "Some Interesting Properties Of Hashing Functions - Validating Passwords",
    duration: "06:31",
    seqNo: 11,
    bookId: 6
  },

  // PWA course
  22: {
    id: 22,
    description: "Course Kick-Off - Install Node, NPM, IDE And Service Workers Section Code",
    duration: "07:19",
    seqNo: 1,
    bookId: 7
  },

  23: {
    id: 23,
    description: "Service Workers In a Nutshell - Service Worker Registration",
    duration: "6:59",
    seqNo: 2,
    bookId: 7
  },

  24: {
    id: 24,
    description: "Service Workers Hello World - Lifecycle Part 1 and PWA Chrome Dev Tools",
    duration: "7:28",
    seqNo: 3,
    bookId: 7
  },

  25: {
    id: 25,
    description: "Service Workers and Application Versioning - Install & Activate Lifecycle Phases",
    duration: "10:17",
    seqNo: 4,
    bookId: 7
  },

  26: {
    id: 26,
    description: "Downloading The Offline Page - The Service Worker Installation Phase",
    duration: "09:50",
    seqNo: 5,
    bookId: 7
  },

  27: {
    id: 27,
    description: "Introduction to the Cache Storage PWA API",
    duration: "04:44",
    seqNo: 6,
    bookId: 7
  },

  28: {
    id: 28,
    description: "View Service Workers HTTP Interception Features In Action",
    duration: "06:07",
    seqNo: 7,
    bookId: 7
  },

  29: {
    id: 29,
    description: "Service Workers Error Handling - Serving The Offline Page",
    duration: "5:38",
    seqNo: 8,
    bookId: 7
  },

  // Serverless SPA with Firebase
  30: {
    id: 30,
    description: "Development Environment Setup",
    duration: "5:38",
    seqNo: 1,
    bookId: 1
  },

  31: {
    id: 31,
    description: "Introduction to the Firebase Ecosystem",
    duration: "5:12",
    seqNo: 2,
    bookId: 1
  },

  32: {
    id: 32,
    description: "Importing Data into Firestore",
    duration: "4:07",
    seqNo: 3,
    bookId: 1
  },

  33: {
    id: 33,
    description: "Firestore Documents in Detail",
    duration: "7:32",
    seqNo: 4,
    bookId: 1
  },

  34: {
    id: 34,
    description: "Firestore Collections in Detail",
    duration: "6:28",
    seqNo: 5,
    bookId: 1
  },

  35: {
    id: 35,
    description: "Firestore Unique Identifiers",
    duration: "4:38",
    seqNo: 6,
    bookId: 1
  },

  36: {
    id: 36,
    description: "Querying Firestore Collections",
    duration: "7:54",
    seqNo: 7,
    bookId: 1
  },

  37: {
    id: 37,
    description: "Firebase Security Rules In Detail",
    duration: "5:31",
    seqNo: 8,
    bookId: 1
  },

  38: {
    id: 38,
    description: "Firebase Cloud Functions In Detail",
    duration: "8:19",
    seqNo: 9,
    bookId: 1
  },

  39: {
    id: 39,
    description: "Firebase Storage In Detail",
    duration: "7:05",
    seqNo: 10,
    bookId: 1
  },


// SPA Testing
  40: {
    id: 40,
    description: "SPA Testing - Helicopter View",
    duration: "5:38",
    seqNo: 1,
    bookId: 12
  },

  41: {
    id: 41,
    description: "Setting Up the Development Environment",
    duration: "5:12",
    seqNo: 2,
    bookId: 12
  },

  42: {
    id: 42,
    description: "Introduction to Jasmine, Spies and specs",
    duration: "4:07",
    seqNo: 3,
    bookId: 12
  },

  43: {
    id: 43,
    description: "Introduction to Service Testing",
    duration: "7:32",
    seqNo: 4,
    bookId: 12
  },

  44: {
    id: 44,
    description: "Settting up the SPA TestBed",
    duration: "6:28",
    seqNo: 5,
    bookId: 12
  },

  45: {
    id: 45,
    description: "Mocking SPA HTTP requests",
    duration: "4:38",
    seqNo: 6,
    bookId: 12
  },

  46: {
    id: 46,
    description: "Simulating Failing HTTP Requests",
    duration: "7:54",
    seqNo: 7,
    bookId: 12
  },

  47: {
    id: 47,
    description: "Introduction to SPA Component Testing",
    duration: "5:31",
    seqNo: 8,
    bookId: 12
  },

  48: {
    id: 48,
    description: "Testing SPA Components without the DOM",
    duration: "8:19",
    seqNo: 9,
    bookId: 12
  },

  49: {
    id: 49,
    description: "Testing SPA Components with the DOM",
    duration: "7:05",
    seqNo: 10,
    bookId: 12
  },


  // Ngrx
  50: {
    id: 50,
    description: "Welcome to the SPA Ngrx",
    duration: "6:53",
    seqNo: 1,
    bookId: 4

  },
  51: {
    id: 51,
    description: "The SPA Ngrx Architecture - Helicopter View",
    duration: "5:52",
    seqNo: 2,
    bookId: 4
  },
  52: {
    id: 52,
    description: "The Origins of Flux - Understanding the Famous Facebook Bug Problem",
    duration: "8:17",
    seqNo: 3,
    bookId: 4
  },
  53: {
    id: 53,
    description: "Custom Global Events - Why Don\'t They Scale In Complexity?",
    duration: "7:47",
    seqNo: 4,
    bookId: 4
  },
  54: {
    id: 54,
    description: "The Flux Architecture - How Does it Solve Facebook Counter Problem?",
    duration: "9:22",
    seqNo: 5,
    bookId: 4
  },
  55: {
    id: 55,
    description: "Unidirectional Data Flow And The SPA Development Mode",
    duration: "7:07",
    seqNo: 6,
    bookId: 4
  },

  56: {
    id: 56,
    description: "Dispatching an Action - Implementing the Login Component",
    duration: "4:39",
    seqNo: 7,
    bookId: 4
  },
  57: {
    id: 57,
    description: "Setting Up the Ngrx DevTools - Demo",
    duration: "4:44",
    seqNo: 8,
    bookId: 4
  },
  58: {
    id: 58,
    description: "Understanding Reducers - Writing Our First Reducer",
    duration: "9:10",
    seqNo: 9,
    bookId: 4
  },
  59: {
    id: 59,
    description: "How To Define the Store Initial State",
    duration: "9:10",
    seqNo: 10,
    bookId: 4
  },

  // NestJs

  60: {
    id: 60,
    description: "Introduction to NestJs",
    duration: "4:29",
    seqNo: 1,
    bookId: 14
  },
  61: {
    id: 61,
    description: "Development Environment Setup",
    duration: "6:37",
    seqNo: 2,
    bookId: 14
  },
  62: {
    id: 62,
    description: "Setting up a MongoDB Database",
    duration: "6:38",
    seqNo: 3,
    bookId: 14
  },
  63: {
    id: 63,
    description: "CRUD with NestJs - Controllers and Repositories",
    duration: "12:12",
    seqNo: 4,
    bookId: 14
  },
  64: {
    id: 64,
    description: "First REST endpoint - Get Alls",
    duration: "3:42",
    seqNo: 5,
    bookId: 14
  },
  65: {
    id: 65,
    description: "Error Handling",
    duration: "5:15",
    seqNo: 6,
    bookId: 14
  },
  66: {
    id: 66,
    description: "NestJs Middleware",
    duration: "7:08",
    seqNo: 7,
    bookId: 14
  },
  67: {
    id: 67,
    description: "Authentication in NestJs",
    duration: "13:22",
    seqNo: 8,
    bookId: 14
  },
  68: {
    id: 68,
    description: "Authorization in NestJs",
    duration: "6:43",
    seqNo: 9,
    bookId: 14
  },
  69: {
    id: 69,
    description: "Guards & Interceptors",
    duration: "8:16",
    seqNo: 10,
    bookId: 14
  },

  // Stripe

  70: {
    id: 70,
    description: "Introduction to Stripe Payments",
    duration: "03:45",
    seqNo: 0,
    bookId: 16
  },
  71: {
    id: 71,
    description: "The advantages of Stripe Checkout",
    duration: "08:36",
    seqNo: 1,
    bookId: 16
  },
  72: {
    id: 72,
    description: "Setting up the development environment",
    duration: "09:10",
    seqNo: 2,
    bookId: 16
  },
  73: {
    id: 73,
    description: "Creating a server Checkout Session",
    duration: "07:20",
    seqNo: 3,
    bookId: 16
  },
  74: {
    id: 74,
    description: "Redirecting to the Stripe Checkout page",
    duration: "11:47",
    seqNo: 4,
    bookId: 16
  },
  75: {
    id: 75,
    description: "Order fulfillment webhook",
    duration: "06:30",
    seqNo: 5,
    bookId: 16
  },
  76: {
    id: 76,
    description: "Installing the Stripe CLI",
    duration: "4:13",
    seqNo: 6,
    bookId: 16
  },
  77: {
    id: 77,
    description: "Firestore Security Rules for protecting Premium content",
    duration: "05:47",
    seqNo: 7,
    bookId: 16
  },
  78: {
    id: 78,
    description: "Stripe Subscriptions with Stripe Checkout",
    duration: "05:17",
    seqNo: 8,
    bookId: 16
  },
  79: {
    id: 79,
    description: "Stripe Subscription Fulfillment",
    duration: "07:50",
    seqNo: 9,
    bookId: 16
  },


  // Reactive SPA

  80: {
    id: 80,
    description: "Introduction to Reactive Programming",
    duration: "03:45",
    seqNo: 0,
    bookId: 17,
    videoId: "Df1QnesgB_s",
  },
  81: {
    id: 81,
    description: "Introduction to RxJs",
    duration: "08:36",
    seqNo: 1,
    bookId: 17,
    videoId: "8m5RrAtqlyw",
  },
  82: {
    id: 82,
    description: "Setting up the development environment",
    duration: "09:10",
    seqNo: 2,
    bookId: 17,
    videoId: "3fDbUB-nKqc",
  },
  83: {
    id: 83,
    description: "Designing and building a Service Layer",
    duration: "07:20",
    seqNo: 3,
    bookId: 17,
    videoId: "",
  },
  84: {
    id: 84,
    description: "Stateless Observable Services",
    duration: "11:47",
    seqNo: 4,
    bookId: 17,
    videoId: "qvDPnRs_ZPA",
  },
  85: {
    id: 85,
    description: "Smart vs Presentational Components",
    duration: "06:30",
    seqNo: 5,
    bookId: 17,
    videoId: "5bsZJGAelFM",
  },
  86: {
    id: 86,
    description: "Lightweight state management",
    duration: "4:13",
    seqNo: 6,
    bookId: 17,
    videoId: "9m3_HHeP9Ko",
  },
  87: {
    id: 87,
    description: "Event bubbling anti-pattern",
    duration: "05:47",
    seqNo: 7,
    bookId: 17,
    videoId: "PRQCAL_RMVo",
  },
  88: {
    id: 88,
    description: "Master detail with cached master table",
    duration: "05:17",
    seqNo: 8,
    bookId: 17,
    videoId: "du4ib4jBUG0"
  },
  89: {
    id: 89,
    description: "Error handling",
    duration: "07:50",
    seqNo: 9,
    bookId: 17,
    videoId: "8m5RrAtqlyw"
  }

};

export const USERS = {
  1: {
    id: 1,
    email: 'test@angular.io',
    password: 'test',
    pictureUrl: 'https://lh3.googleusercontent.com/-1pUNnTB3vaA/AAAAAAAAAAI/AAAAAAAAAAA/ACHi3rdn4uEc0ti8YE4Uuw6_Kz04tVe2Mg.CMID/s32-c/photo.jpg'
  }
};
